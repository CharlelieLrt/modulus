...
class UNet(Module):  # TODO a lot of redundancy, need to clean up
    ...
    setting the noise level and the class labels to zero. Net
    models.
    ...
    - model_type: Literal['SongUNet', 'SongUNetPosEmbd', 'SongUNetPosLtEmbd', 'DhariwalUNet']
    ...
    Forward
    -------
    The model should be called with `output = model(x, img_lr, **model_kwargs)`

    Inputs:
        - x : torch.Tensor
            The input tensor, typically zero-filled, of shape `(B, C_hr, H, W)`.
        - img_lr : torch.Tensor
            Conditioning image of shape `(B, C_lr, H, W)`.
        - **model_kwargs : dict
    ...
    Returns:
        - output : torch.Tensor
            Output tensor of shape `(B, C_hr, H, W)`.

    References
    ----------
    `Generative Residual Diffusion Modeling for Km-scale Atmospheric
    Downscaling, Mardani et al. <https://arxiv.org/abs/2309.15214>`_
    """
