...
| `GB200` | `results/GB200/` (per-run YAMLs + `_max_domain.yaml` + `summary.yaml` + `plots/` + `calibration/`) | `model_channels=128`, 5-level UNet, 16-channel data, `B_train=4 (DDP x 4)`, `B_infer=1`, `MAX_DOMAIN=1120` (calibrated), `DOMAIN_SWEEP=(64..8192)`, full opt set `{amp_bf16, compile, apex_gn}`. GB200 NVL72 (aarch64) has 4 GPUs/node, so `NPROC_PER_NODE_TRAIN=4` here vs the 8-rank H100 reference (shown as `4x GB200 (DDP)` on the training plots). | The 192 GB HBM pushes the non-MD OOM cutoffs a step above H100/L40s: non-MD settings OOM at d=1024 (training) / d=4096 (inference) / d=2048 (inference+DPS), while MD+opt fits training, inference, and inference+DPS all the way to d=8192. The d=8192 MD+DPS point takes ~4.2h (its guidance autograd graph spans the full global image), so it was captured on `batch_long` rather than the 4h `batch` queue; it fits at ~116 GB (59% util) on the 184 GB GPU, where L40s OOMs. `apex.contrib.group_norm` is available on aarch64, so the full-opt set matches the reference. |
