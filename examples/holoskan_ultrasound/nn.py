...
class DiffusionFWINet(Module):
    ...
    def forward(
        ...
        if self.unconditional:
            ...
            pos_emb = pos_embd.permute(2, 1, 0)  # (W_x, H, N_grid)
            grid_embed = self.grid_to_receivers(pos_emb)  # (W_x, Cg, N_grid)
            grid_embed = rearrange(grid_embed, "w g n -> (g n) w")  # (Cg * N_grid, W_x)
            grid_embed = grid_embed[None, :, None, :].expand(
                B, -1, T, -1
            )  # (B, Cg * N_grid, T, W_x)
            # Interpolate grid_embed to match y's width (W_y) if different from W_x
            W_y = y.shape[-1]
            if grid_embed.shape[-1] != W_y:
                grid_embed = F.interpolate(
                    grid_embed, size=(T, W_y), mode="bilinear", align_corners=False
                )  # (B, Cg * N_grid, T, W_y)
            y = torch.cat((y, grid_embed), dim=1)  # (B, C_y + Cg * N_grid, T, W_y)

            # Encode seismic data
            y_encoded = self.time_signal_encoder(y)  # (B, C_hidden//2, H, W_y)

            # Interpolate y_encoded to match x's spatial dimensions if different
            if y_encoded.shape[-1] != W:
                y_encoded = F.interpolate(
                    y_encoded, size=(H, W), mode="bilinear", align_corners=False
                )  # (B, C_hidden//2, H, W)
